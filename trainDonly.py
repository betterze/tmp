...
def InitialP():
    ...
    tmp[2,2]=1.0
    tmp=logm(tmp)
    tmp1=np.zeros([opt.batchSize,3,3])
    tmp1[:,:,:]=tmp
    tmp1=tmp1.reshape(opt.batchSize,9)
    tmp1=tmp1[:,:6]
    tmp1=np.float32(tmp1)
    return tmp1
...
#	pPertFG = opt.pertFG*tf.random_normal([opt.batchSize,opt.warpDim])
	pPertFG =tf.constant(InitialP(),tf.float32)
	# ------ define GP and D ------
	...
	pMtrx = warp.vec2mtrx(opt,pPertFG)
...
#		tmp=sess.run(pPertFG,feed_dict=batch)
#		print('pPertFG',tmp)
		tmp=sess.run(pMtrx,feed_dict=batch)
		print('pMtrx',tmp)
